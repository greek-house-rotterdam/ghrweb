...
|------|-------------|-----------------|
| **admins** | Maintain | Push to `main`, merge PRs, manage repo settings |
| **editors** | Write | Create branches, open PRs — changes require admin approval (via branch protection) |
...
- Require pull request before merging (1 approval from admins)
- No direct pushes to `main` (except Owners and admins team)
- This enforces the editorial workflow: editors create content via Decap CMS → PR is opened → admin reviews and merges → site deploys
...
- **Admins** can push directly to `main` and merge PRs
