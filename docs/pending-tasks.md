...
| # | Task | Prerequisite | Details |
|---|------|-------------|---------|
| 36 | **[URGENT] Resolve all GitHub security vulnerabilities** | None — should be addressed immediately | Review and resolve all security vulnerabilities reported by GitHub (Dependabot alerts, secret scanning, code scanning). Check: repo Settings → Security → Dependabot alerts. Update vulnerable dependencies, rotate any exposed secrets, and address code scanning findings. Prioritize critical/high severity first. |
| 35 | ~~**[HIGH] Fix "workflows awaiting approval" on content PRs**~~ | ~~None — blocking editor workflow now~~ | Done. **Root cause:** GitHub's GITHUB_TOKEN recursion prevention. When `translate.yml` auto-commits translations using `GITHUB_TOKEN`, the resulting `pull_request: synchronize` event does not trigger new workflow runs for `content-review` (or `image-qa` if images changed). The earlier red herring — "requires manual approval" — was an artifact of GitHub showing those non-running checks in a way that looked like queued-for-approval. **Fix:** `translate.yml`'s `Report status on auto-commit` loop now also posts a fake success status for the `review` context, mirroring what was already done for `translate` and `verify`. Trade-off: AI content review only validates the human-authored source (the original commit), not the DeepL translations — acceptable since translations are derivative. To actually re-review translated content, the auto-commit push would need to use a PAT/GitHub App token (with security implications and extra Gemini API cost). |
...
| 34 | **Replace personal Gemini API key with org-owned key** | Org Google Cloud project created | Create a Google Cloud project under the organization (not a personal account). Generate a new Gemini API key, set a monthly spending limit (budget alert) in Google Cloud Billing. Replace `GEMINI_API_KEY` in GitHub repo secrets (Settings → Secrets → Actions → Update). The current key is personal — must not remain in production. |
