...
| 35 | **[HIGH] Fix "workflows awaiting approval" on content PRs** | None — blocking editor workflow now | The `content-review` and `image-qa` workflows require manual approval ("Approve workflows to run") on every PR created via Decap CMS `/admin`, even when the PR author is an org member/owner. The `translate` workflow is not affected. **What was tried (all failed):** 1) Org setting → "Require approval for first-time contributors who are new to GitHub" (least restrictive). 2) Org setting → "Allow all actions and reusable workflows". 3) Approving workflows on multiple previous PRs. 4) Checking Actions tab for enable banners. 5) Org setting → Workflow permissions changed to "Read and write permissions". **Impact:** Every content PR requires a maintainer to manually click "Approve and run" before these two workflows execute. The translate workflow runs automatically. **Possible causes to investigate:** GitHub may treat Decap CMS OAuth-created PRs differently from direct user PRs; the `labeled` event type may trigger different approval behavior; the recent addition of `astral-sh/setup-uv@v7` to these workflows (translate already had it) may be a factor. |
