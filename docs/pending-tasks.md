...
| 35 | ~~**[HIGH] Fix "workflows awaiting approval" on content PRs**~~ | ~~None — blocking editor workflow now~~ | Done. **Root cause:** GitHub's GITHUB_TOKEN recursion prevention. When `translate.yml` auto-commits translations using `GITHUB_TOKEN`, the resulting `pull_request: synchronize` event does not trigger new workflow runs for `content-review`. The earlier framing — "requires manual approval" — was an artifact of GitHub's PR-checks UI hiding workflows that didn't re-trigger on the head SHA. **Fix:** `translate.yml`'s `Report status on auto-commit` step now **carries forward** the parent commit's `review` conclusion to the bot's commit, rather than synthesizing success. If review failed on the source commit (e.g. Gemini outage), the failure surfaces on the head SHA too. See [`docs/workflows.md`](./workflows.md#review-status-on-the-bots-commit-carry-forward). Trade-off: AI review only validates human-authored content, not DeepL output — acceptable since translations are derivative; revisit with a PAT/App-token push if translation quality issues emerge. |
